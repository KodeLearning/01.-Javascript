...
for (const worker of datos) {
  let i = 0
  while (i < worker.habilidades.length) {
    if (worker.habilidades[i] === 'JavaScript') {
      desarrolladoresJavascript.push(worker)
    }
    i++
  }

  for (const project of worker.proyectos) {
    nombresProyectos.push(project.nombre)
  }
}
console.log('Desarrolladores de Javascript:', desarrolladoresJavascript)
console.log('Proyectos activos:', nombresProyectos)
